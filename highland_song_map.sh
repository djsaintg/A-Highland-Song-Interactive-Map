...
<html lang="en">
  <head>
    ...
    <title>A Highland Song — Peak Travel Map</title>
...
function normalizeEntry(x: any): NoteEntry {
  ...
  const buildJSON = useCallback(() => JSON.stringify({ app: "a-highland-song-peak-map", version: "1.1.0", exported: new Date().toISOString(), notes, blessingSelections }, null, 2), [notes, blessingSelections]);
  ...
  const importNotes = useCallback((file: File) => {
    ...
    r.onload = () => { try {
      ...
      if ((!incomingNotes || typeof incomingNotes !== "object") && (!incomingBlessings || typeof incomingBlessings !== "object")) { flash("Invalid file"); return; }
      let addedNotes = 0, addedBlessings = 0;
        ...
        for (const [id, arr] of Object.entries(incomingNotes as Record<string, unknown>)) {
          ...
          for (const rawItem of arr) {
            ...
            if (normalized.text.trim() && !merged.some(existing => existing.text === normalized.text && existing.category === normalized.category)) {
              merged.push(normalized); addedNotes++;
            }
          }
      ...
      setNotes(mergedNotes); setBlessingSelections(mergedBlessings);
        ...
        <div className="overflow-x-auto">
          <svg width={SVG_W} height={SVG_H} viewBox={"0 0 "+SVG_W+" "+SVG_H} style={{display:"block"}}>
            ...
            {connections.map((conn,i) => {
              ...
              const rawDx=rawTx-rawFx, rawDy=rawTy-rawFy, rawLength=Math.hypot(rawDx,rawDy)||1;
              const lane=conn.lane??0, perpX=-rawDy/rawLength, perpY=rawDx/rawLength;
              const fx=rawFx+perpX*lane, fy=rawFy+perpY*lane, tx=rawTx+perpX*lane, ty=rawTy+perpY*lane;
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {ap.blessings.length > 0 ? ap.blessings.map(blessing => {
                    ...
                    return (
                      <button key={blessing} type="button" disabled={!selected}
                        ...
                        className={"text-xs px-1.5 py-0.5 rounded-full border transition-colors "+(isLeft?"bg-emerald-700/70 border-emerald-400 text-emerald-50 font-semibold":"bg-amber-900/40 border-amber-700/50 text-amber-200")+(selected?" cursor-pointer hover:border-emerald-400":" cursor-default")}>
                        {isLeft&&<span className="mr-1">&#x2713;</span>}{blessing}
                      </button>
                    );
                  }):<span className="text-slate-600 text-xs italic">Not blessable</span>}
                </div>
                {ap.blessings.length>0&&<p className="text-slate-500 text-[10px] mt-1.5 italic">{blessingSelections[ap.id]?"Green marks the offering currently left at this peak.":selected?"Select the offering left at this peak.":"Pin this peak to track its offering."}</p>}
