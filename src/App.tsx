...
function findPath(startId: string, endId: string): string[] | null {
    ...
    // Build directed adjacency: both "both" and "one-way" edges
...
// Tooltip width constant used for flip calculation
const TIP_W = 400;
...
export default function App() {
    ...
    // Track raw viewport mouse position at the window level
    useEffect(() => {
    ...
    // Route-aware highlight helpers
    // A peak is "on path" if it appears in routePath
    const onPath = useCallback((id: string) => !!routePath && routePath.includes(id), [routePath]);
    // An edge is "on path" if it connects two consecutive path nodes in the correct direction
    ...
    // Flip tooltip left if it would overflow the right edge of the viewport
    const OFFSET = 18;
    const tipLeft = mouse.x + OFFSET + TIP_W > window.innerWidth
    ? mouse.x - TIP_W - OFFSET
    : mouse.x + OFFSET;
    // Flip tooltip up if it would overflow the bottom
    const TIP_H_APPROX = 260;
    const tipTop = mouse.y + OFFSET + TIP_H_APPROX > window.innerHeight
    ? mouse.y - TIP_H_APPROX - OFFSET
    : mouse.y + OFFSET;
    ...
    return (
        ...
        {routePath ? routePath.length-1+" stops found" : "Find the best path between two peaks"}
        ...
        {plannerOpen && (
            ...
            <select
            value={routeStart}
            onChange={e => { setRouteStart(e.target.value); clearRoute(); }}
            ...
            <select
            value={routeEnd}
            onChange={e => { setRouteEnd(e.target.value); clearRoute(); }}
        ...
        {connections.map((conn,i) => {
            ...
            // Route-path highlighting takes priority over hover highlighting
            const pathEdge = edgeOnPath(conn.from, conn.to, conn.type);
            const hl = iC(conn.from,conn.to);
            const dim = routePath
            ? !pathEdge          // dim everything off-path when a route is shown
            : (active && !hl);   // normal hover dim
            const st = pathEdge
            ? "#a78bfa"          // bright violet for route path
            : dim
            ? "rgba(255,255,255,0.04)"
            : isOW ? (hl?"#fdba74":"#f97316") : (hl?"#6ee7b7":"#34d399");
            const pathMarker = isOW ? "url(#route-end)" : undefined;
            const mE = pathEdge ? pathMarker??"url(#route-end)"
            : isOW?(hl?"url(#aoh)":"url(#ao)"):(hl?"url(#abeh)":"url(#abe)");
            const mS = pathEdge ? (isOW?undefined:"url(#route-start)")
            : isOW?undefined:(hl?"url(#absh)":"url(#abs)");
        ...
        {peaks.map(p => {
            ...
            // Dim logic: route mode overrides hover mode
            const isDim = routePath
            ? !isOnPath
            : (active && !isHL);
            const bx=x+6, by=pcy-BADGE_SIZE/2, tx2=bx+BADGE_SIZE+6, tw=NODE_W-BADGE_SIZE-18;
            // Determine node border colour
            const borderCol = isPathStart ? "#4ade80"
            : isPathEnd   ? "#f472b6"
            : isOnPath    ? "#a78bfa"
            : isPinned    ? "#fff"
            : isHL        ? m?.nBr??"#666"
            : isDim       ? "rgba(255,255,255,0.06)"
            : m?.nBr??"#666";
            ...
            return (
                ...
                {/* Glow ring for path start/end/via nodes */}
                {isOnPath && (
                    <rect x={x-4} y={y-4} width={NODE_W+8} height={NODE_H+8} rx={13}
                    fill="none"
                    stroke={isPathStart?"#4ade80":isPathEnd?"#f472b6":"#7c3aed"}
                ...
                {/* Step number badge for path nodes */}
                {isOnPath && routePath && (
        ...
        {/* ── Floating tooltip — position:fixed so it follows the cursor regardless of scroll ── */}
        {ap && (
            <div
            style={{
                position: "fixed",
                ...
                width: TIP_W,
                zIndex: 9999,
                // While hovering (not pinned) the card must not intercept mouse events or
                // it will cause a flicker as the cursor enters the card and "leaves" the peak.
            ...
            {ap.blessings.length > 0
                ? ap.blessings.map(b => (
                    <span key={b} className="bg-amber-900/40 border border-amber-700/50 text-amber-200 text-xs px-1.5 py-0.5 rounded-full">{b}</span>
                ))
                : <span className="text-slate-600 text-xs italic">Not blessable</span>
            }
            </div>
            {ap.blessingNote && (
                <p className="text-slate-400 text-xs mt-1.5 italic leading-snug">&#8505; {ap.blessingNote}</p>
            )}
            </div>
            ...
            {!selected && (
                <p className="text-slate-600 text-xs text-center mt-2 border-t border-slate-700 pt-2">
                Click to pin
                </p>
            )}
            </div>
